...
library(tidyverse)
library(flextable)
# tm = read_trialmaster("path/to/zip")
tm = edc_example_ae()
load_list(tm)
...
La macro `AE_grades` est traduite en deux fonctions : `ae_table_grade_max()` et `ae_table_grade_n()`. Ces fonctions retourne des objets de classe `crosstable`. 
...
### Sortie *AE_grades1* : Table des grades maximum par patient
...
### Sortie *AE_grades2*: Table des grades maximum par patient stratifié sur le bras
...
### Sortie *Bonus*: Graphique des AE maximums

```{r, fig.asp = 0.5, fig.width = 7, out.width = "100%"}
ae_plot_grade_max(df_ae=ae, df_enrol=enrolres, type = c("stack", "dodge", "fill"))  & labs(fill="Arm")
```

### Sortie *AE_grades3*: Table de tous les grades pour chaque patient
...
### Sortie *AE_grades4*: Table des grades maximum par patient, filtrée sur les SAE

La fonction ne prend pas en charge les SAE directement, il suffit de filtrer la table AE en amont.
Pour l'exemple, on va considérer comme SAE les AE de grade 3+.
