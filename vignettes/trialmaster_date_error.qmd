...
format: 
  html:
    toc: false
vignette: >
  %\VignetteIndexEntry{Table Reading Date Error}
...
In this situation, the **Text** format may incorrectly be applied to date fields.
This makes the whole table unreadable in R, throwing the following error:
...
2. Identify the **date-type variables** that are currently in:  
   ...
   ![Identify variables with incorrect format](img/tm_date_error_2.png)
