...
As in previous vignettes, we will be using `edc_example()`, but you should use EDC reading functions (see `vignette("reading")`)
...
You can write these checks as per your Data Validation Plan, and they should run after each export.

If one check is so mandatory that you cannot work in a database where it fails, use `edc_data_stop()` instead.

At some point, you can use `edc_data_warnings()` to get a summary.
