...
#' @param origin a variable to consider as the time 0, given as "dataset$column"
...
swimmerplot = function(.lookup=getOption("edc_lookup", NULL), id="SUBJID", 
  ...
  if(!is.null(origin)){
    ...
    dat_origin = dat %>% 
      filter(dataset==origin2[1], name==origin2[2]) %>% 
      select(id, origin=value)
    if(nrow(dat_origin)==0){
      cli_abort(c("{.arg origin} is wrong somehow", i="sorry for the unhelpful message"))
    }
    dat = dat %>% 
      left_join(dat_origin, by="id") %>% 
      mutate(
        value_bak = value,
        value = as.numeric(value-origin)
      )
